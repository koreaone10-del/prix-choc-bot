...
// اقرأ بيانات الدخول من متغيرات البيئة (أضفها في Render)
const BABA_EMAIL    = process.env.BABA_EMAIL    || '';
const BABA_PASSWORD = process.env.BABA_PASSWORD || '';

// ─────────────────────────────────────────────
// خريطة الولايات (58 ولاية) - صحيحة 100%
// ─────────────────────────────────────────────
const wilayasMap = {
  "1":  "أدرار",
  "2":  "الشلف",
...
app.post('/automate-order', async (req, res) => {
  ...
  // رد فوري لـ Vercel (لتجنب Timeout)
  ...
  try {
    ...
    // انتظار الجلسة (5 ثوانٍ)
    await new Promise(r => setTimeout(r, 5000));

    const urlAfterLogin = page.url();
    if (urlAfterLogin.includes('/login')) {
      throw new Error('فشل تسجيل الدخول - تحقق من البريد/كلمة المرور');
    }
    ...
    if (!clicked) {
      ...
      if (!clicked2) {
        ...
        throw new Error('زر "ابدأ البيع الآن" غير موجود - تم حفظ صورة تشخيص');
      }
    }
